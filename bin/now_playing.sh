...
if is_app_running("Dezzer") then
    tell application "Dezzer" to set dezzer_state to (player state as text)
end if
...
else if dezzer_state is equal to "playing" then
    tell application "Dezzer"
    set track_name to name of current track
    set artist_name to artist of current track
    return track_name & " - #[bold]" & artist_name & "#[nobold]"
    end tell
